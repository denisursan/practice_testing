...
describe("Traversing DOM elements in Cypress", () => {
    ...
    it("eq() to retrieve a specific element based on index", () => {
        cy.get('.traversal-drinks-list li').eq(0).should('have.text', 'Coffee')
        cy.get('.traversal-drinks-list >*').eq(0).should('have.text', 'Coffee')
    });

    it("filter() to retrieve DOM elements that match a specific selector", () => {
        cy.get('.btn-group-toggle >*').filter('.active').click().should('contain', 'Button-1')
    });

    it("find() to retrieve DOM elements of a given selector", () => {
        cy.get('.traversal-pagination').find('li').find('a').eq(1).click()
    ...
    cy.get('.menu').eq(0).prevUntil('.sales').should('have.length','1')
    });
